...
{
    int i;
    for(i = 0;cardapios[i].getProduto().toLower() != a.getProduto().toLower();i++){
    }
    return i;
}
...
void produtos::limpardados()
{
    cardapios.erase(cardapios.begin(),cardapios.end());
}

void produtos::apagarproduto(Cardapio a)
{
    int i = procurarcardapio(a);
    cardapios.erase(cardapios.begin()+i);
}

bool produtos::testeproduto(Cardapio a)
{
    for(int i = 0;i < cardapios.size();i++){
        if(a.getProduto().toLower() == cardapios[i].getProduto().toLower()){
        return 1;
        }
    }
    return 0;
